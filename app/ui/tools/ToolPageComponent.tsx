    ...
    return (
        <section className="py-3 md:mx-10 space-y-5">
            ...
            <div className="flex flex-col lg:flex-row gap-2.5 items-start lg:items-center">
                <Dropdown options={categories} selected={selectedCategory} onSelect={setSelectedCategory} width="[150px]" />
                <Dropdown options={makes} selected={selectedMake} onSelect={setSelectedMake} width="[180px]" />
                <Dropdown options={models} selected={selectedModel} onSelect={setSelectedModel} width="[200px]" />
                <Dropdown options={distances} selected={selectedDistance} onSelect={setSelectedDistance} width="[150px]" />
            </div>
            <div>
                <h2 className="leading-7 text-2xl font-bold">{activeTools} available tools</h2>
            </div>

            {/* tools grid */}
            <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 ${collapsed && 'xl:grid-cols-6'} gap-4`}>
                {tools.map(tool => (
                    <ToolTab 
                    key={tool.id} 
